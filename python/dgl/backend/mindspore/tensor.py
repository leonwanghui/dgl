...
import mindspore.context as msctx
from mindspore import Tensor, SparseTensor

from ...base import dgl_warning

if LooseVersion(ms.__version__) < LooseVersion("1.0.0"):
    dgl_warning("Detected an old version of MindSpore. Suggest using mindspore>=1.0.0 "
...
def data_type_dict():
    ...
    >>> import mindspore as ms
    >>> def data_type_dict():
    >>>   return { 'float16' : ms.float16, 'float32' : ms.float32, ... }

    After the module is initialized.

    >>> import backend as F
    >>> F.float16  # this will point to mindspore.float16
...
def cpu():
    ...
    msctx.set_context(device_target="CPU")
    return msctx._context()
...
def sparse_matrix(data, index, shape, force_format=False):
    ...
    """
    return SparseTensor(index, data, shape)
...
def device_type(ctx):
    ...
    str
    """
    return msctx.get_context("device_target")
...
def device_id(ctx):
    ...
    int
        The device index.
    """
    return msctx.get_context("device_id")
...
def to_backend_ctx(dglctx):
    ...
    if dev_type == 1:
        ...
        msctx.set_context(device_target="GPU")
    else:
...
def copy_to(input, ctx, **kwargs):
    ...
    """
    return Tensor(input, input.dtype)
...
def sum(input, dim, keepdims=False):
    ...
    Tensor
        A framework-specific tensor.
    """
    data = input.asnumpy().sum(axis=dim, keepdims=keepdims)
    return Tensor(data, input.dtype)
...
def cumsum(input, dim):
    ...
    Tensor
        A framework-specific tensor.
    """
    data = input.asnumpy().cumsum(axis=dim)
    return Tensor(data, input.dtype)
...
def max(input, dim):
    ...
    Tensor
        A framework-specific tensor.
    """
    data = input.asnumpy().max(axis=dim)
    return Tensor(data, input.dtype)
...
def min(input, dim):
    ...
    Tensor
        A framework-specific tensor.
    """
    idatanput = input.asnumpy().min(axis=dim)
    return Tensor(data, input.dtype)
...
def argsort(input, dim, descending):
    ...
    Tensor
        A framework-specific tensor.
    """
    data = input.asnumpy().argsort(axis=dim, order=descending)
    return Tensor(data, input.dtype)
...
def reshape(input, shape):
    ...
    Tensor
        The reshaped tensor.
    """
    data = input.asnumpy().reshape(shape=shape)
    return Tensor(data, input.dtype)
