...
def _reduce_grad(grad, shape):
    ...
    reduce_idx = np.nonzero(np.array(grad_shape) - np.array(in_shape), as_tuple=False)
    ...
    def backward(ctx, dZ):
        ...
        if op != 'copy_rhs' and ctx.needs_input_grad[3]:
            ...
            if reduce_op == 'sum':
                ...
                dX = np.zeros((X.shape[0],) + dZ.shape[1:],
        ...
        if op != 'copy_lhs' and ctx.needs_input_grad[4]:
            if reduce_op == 'sum':
                ...
                dY = np.zeros((Y.shape[0],) + dZ.shape[1:],
...
class EdgeSoftmax(nn.Cell):
    ...
    def forward(ctx, gidx, score, eids, norm_by):
        ...
        score = np.exp(_gsddmm(gidx, 'sub', score, score_max, 'e', 'v'))
...
class SegmentReduce(nn.Cell):
    ...
    def backward(ctx, dy):
        ...
        if op == 'sum':
            ...
            indices.scatter_add_(0, offsets, np.ones_like(offsets))
